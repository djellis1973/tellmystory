...
def save_response(session_id, question, answer, answer_index=None):
    ...
    # FIXED: Apply spellcheck BEFORE saving the response
...
with st.sidebar:
    ...
    # 10. Export Options - FIXED: Export format for publisher
    ...
    if st.session_state.logged_in and st.session_state.user_id:
        # FIXED: Prepare data in the CORRECT format for publisher - LIST format with question+answer pairs
        stories_list = []
        ...
        for session in SESSIONS:
            ...
            if session_data.get("questions"):
                # Group answers by question and take only the best/longest answer for each question
                questions_dict = {}
                for answer_key, answer_data in session_data.get("questions", {}).items():
                    ...
                    if question_text not in questions_dict:
                        questions_dict[question_text] = answer_data["answer"]
                    else:
                        # If we have multiple answers, keep the longest one (most complete)
                        current_length = len(questions_dict[question_text])
                        new_length = len(answer_data["answer"])
                        if new_length > current_length:
                            questions_dict[question_text] = answer_data["answer"]
                
                # Add each question with its best answer to the stories list
                for question_text, answer in questions_dict.items():
                    stories_list.append({
        ...
        if stories_list:
            ...
            # Create complete export data in LIST format publisher expects
            complete_data = {
                "user": st.session_state.user_id,
                "user_profile": st.session_state.user_account.get('profile', {}) if st.session_state.user_account else {},
                "stories": stories_list,  # LIST format with question+answer pairs
                "export_date": datetime.now().isoformat(),
                "summary": {
                    "total_stories": total_stories,
                    "total_sessions": len(set(s['session_id'] for s in stories_list))
                }
            }
...
with col1:
    if st.button("💾 Save Answer", key="save_long_form", type="primary", use_container_width=True):
        if user_input:
            # FIX: Spellcheck is now applied in save_response() function
            ...
            # Save the response with auto-incremented index (spellcheck applied inside save_response)
...
# Prepare data for export in CORRECT format - LIST with best answers
for session in SESSIONS:
    session_id = session["id"]
    session_data = st.session_state.responses.get(session_id, {})
    if session_data.get("questions"):
        # Group answers by question and take only the best/longest answer
        questions_dict = {}
        for answer_key, answer_data in session_data.get("questions", {}).items():
            question_text = answer_data.get("question", answer_key.split('_answer_')[0] if '_answer_' in answer_key else answer_key)
            ...
            if question_text not in questions_dict:
                ...
                if len(answer_data["answer"]) > len(questions_dict[question_text]):
                    questions_dict[question_text] = answer_data["answer"]
        
        # Add each question with its best answer
        for question_text, answer in questions_dict.items():
            stories_list.append({
                "question": question_text,
                "answer": answer,
...
if current_user and current_user != "" and stories_list:
    # Count total stories
    total_stories = len(stories_list)
    
    # Create JSON data for the publisher in CORRECT LIST format
